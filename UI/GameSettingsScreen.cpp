...
void GameSettingsScreen::CreateViews() {
	...
	UI::PopupMultiChoice *hwscale = graphicsSettings->Add(new PopupMultiChoice(&g_Config.iAndroidHwScale, gr->T("Display Resolution (HW scaler)"), deviceResolutions, 0, ARRAY_SIZE(deviceResolutions), gr->GetName(), screenManager()));
...
#else
	g_Config.bShowDeveloperMenu = false;
	g_Config.bDumpDecryptedEboot = false;
	g_Config.bEnableLogging = false;
	g_Config.bLogFrameDrops = false;
#endif
