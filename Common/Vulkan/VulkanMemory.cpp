...
VulkanPushBuffer::VulkanPushBuffer(VulkanContext *vulkan, VkBufferUsageFlags usage, size_t size) 
	: vulkan_(vulkan), device_(vulkan->GetDevice()), buf_(0), offset_(0), usage_(usage), size_(size), writePtr_(nullptr) {
...
void VulkanPushBuffer::Destroy() {
	for (BufInfo &info : buffers_) {
		vulkan_->Delete().QueueDeleteBuffer(info.buffer);
		vulkan_->Delete().QueueDeleteDeviceMemory(info.deviceMemory);
	}
	buffers_.clear();
}
